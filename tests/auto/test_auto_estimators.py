...
def test_image_classification_estimator():
    ...
    est = ImageClassificationEstimator({'train': {'epochs': 1, 'batch_size': 8, 'num_workers': 0}, 'gpus': list(range(get_gpu_count()))})
...
def test_center_net_estimator():
    ...
    est = CenterNetEstimator({'train': {'epochs': 1, 'batch_size': 8, 'num_workers': 0}, 'gpus': list(range(get_gpu_count()))})
